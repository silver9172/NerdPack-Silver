...
local dps = {
	...
	{ 'Solar Wrath', '!los & infront & !player.moving & inRange.spell', 'target'},
}
...
local lifebloom = {
	-- Solo
	{ 'Lifebloom', '!los & player.buff.duration <= 4.5 & { partycheck = 1 || { partycheck = 2 & !tank1.exists } || { partycheck = 3 & !tank1.exists }}', 'player'},
	-- 5 man
	{ 'Lifebloom', '!los & tank.buff.duration <= 4.5 & partycheck = 2', 'tank'},
	-- Raid
	-- Only one tank
	{ 'Lifebloom', '!los & tank.buff.duration <= 4.5 & partycheck = 3 & !tank2.exists', 'tank'},
...
local healing = {
	...
	{ 'Regrowth', '!los & health <= UI(lrg) & player.buff(Clearcasting).duration >= player.spell(Regrowth).casttime', 'lowest'},
	{ 'Regrowth', '!los & player.buff(Clearcasting).duration >= player.spell(Regrowth).casttime', 'lnbuff(Regrowth)'},
	...
	{ 'Regrowth', '!los & health <= UI(trg)', { 'tank1', 'tank2', 'lowest', 'friendly'}},
}
...
local efflorescence = {
	...
	{ 'Efflorescence', 'area(8,90).heal > 1 & totem(Efflorescence).duration <= 3', { 'target.ground', 'tank.ground', 'tank2.ground', 'friendly.ground'}}, 
}
