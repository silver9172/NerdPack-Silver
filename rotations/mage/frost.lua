...
local survival = {
	{ 'Ice Barrier', 'buff.duration <= 15 & !buff(Refreshment) & player.incdmg(5) >= { player.health.max * 0.25 } > ', 'player'}, 
	{ 'Freeze', 'area(8).enemmies >= 3 & bosscheck = 0 & range <= 40', 'target.ground'},
}
...
local preCombat = {
	{ 'Frostbolt', 'player.spell.casttime < dbm(Pull in) & dbm(Pull in) > 0', 'target'}, 
}
